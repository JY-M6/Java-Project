...
const handleAvatarSuccess = (response,uploadFile) => {
  // employee.value.image = response.data
     employee.value.image = fixImageUrl(response.data)  // 添加 fixImageUrl 处理
}
// 文件上传之前触发
...
const fixImageUrl = (url) => {
  ...
  // 如果已经是完整的 https:// 开头，直接返回
  if (url.startsWith('https://')) {
    return url
  }
  
  // 如果是 http// 开头（缺少冒号），修复为 https://
  if (url.startsWith('http//')) {
    return url.replace('http//', 'https://')
  }
  
  // 如果包含代理地址，提取真正的 OSS 地址
  if (url.includes('cpolar.top') || url.includes('localhost')) {
  ...
  <el-table :data="empList" border style="width: 100%" @selection-change="handleSelectionChange">
    ...
    <el-table-column label="头像" width="170" align="center">
      <template #default="scope" >
        ...
        <img :src="fixImageUrl(scope.row.image)" alt="Avatar" class="avatar" />
      </template>
    </el-table-column>
          ...
          <el-col :span="24">
            <el-form-item label="头像">
              <el-upload
                ...
                <img v-if="employee.image" :src="fixImageUrl(employee.image)" class="avatar" />
